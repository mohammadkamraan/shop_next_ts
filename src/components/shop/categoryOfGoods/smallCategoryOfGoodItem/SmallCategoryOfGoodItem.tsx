...
import { ICategoryOfGoodInSmallMode } from "../../../../typescript/AppInterfaces";

interface ISmallCategoryOfGoodItem {
  category: ICategoryOfGoodInSmallMode;
}

const SmallCategoryOfGoodItem: FC<ISmallCategoryOfGoodItem> = ({
  category,
}) => {
    ...
    <Link href={categoryLink}>
      <a className='my-2'>
        <figure className='flex flex-col items-center text-center w-40 sm:w-52 text-slate-800 dark:text-slate-400'>
          ...
          <figcaption className='text-sm md:text-base font-bold mt-2'>
            {categoryName}
          </figcaption>
        </figure>
      </a>
